...
router.post("/chat", ensureAuthenticated, async (req, res) => {
  ...
  try {
    ...
    if (!thread) {
      ...
      // Use the first few words of the AI's answer as the default title
    ...
    // Add user and model messages to the thread
    ...
    // If it's a new thread and we got an answer, set the title based on the answer
    if (newThreadCreated && Answer) {
        thread.title = Answer.split(" ").slice(0, 4).join(" ") || "New Chat";
    }

    thread.updatedAt = new Date(); // Update timestamp on every save
    await thread.save(); // Save the updated/new thread
